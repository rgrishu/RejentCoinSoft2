...
const deposit2= async (getPlanId,getThis) => {
    ...
    let inputAmt = getThis;
		...
		var weiAmt = 1000000000*inputAmt;
	...
	var Trade = new web3.eth.Contract(idoContractAbi3, idoContractAddr);
...
const roiData = async() => { 
 ...
 accounts = await ethereum.request({ method: 'eth_requestAccounts' });
	...
	var setHtml = "";
	await Trade.methods.withdrawableMint(userAddr).call({from:userAddr},function(err,result){
...
const claimRoi = async() => {
	...
	var Trade = new web3.eth.Contract(idoContractAbi3,idoContractAddr);
	Trade.methods.withdrawMint().send({from:userAddr},function(err,result){
